...
class AllResultsCsvReporter {
  ...
  onRunComplete(contexts, results) {
    let result4csv = ['"path","suite","case","result"\n'];
    const suites = results.testResults;
    for (const suite of suites) {
      for (const testCase of suite.testResults) {
        result4csv.push(
          `"${suite.testFilePath}", "${testCase.ancestorTitles[0]}","${testCase.title}","${testCase.status}"\n`
        );
      }
    }
    fs.writeFile(
      `${this._options.outputDir}/${this._options.outputFile}`,
      result4csv.join(""),
      (err) => {
        if (err) {
          console.error(err);
